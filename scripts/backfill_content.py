...
def backfill_one(fpath: Path, workers: int = 0) -> tuple[str, str]:
    """Returns (fpath, status) where status in ok|fail|skip|google."""
    try:
    ...
    summary, body = res.get("summary", ""), res.get("content", "")
    if not summary or not body:
        return str(fpath), "fail"
    new_content = append_body(inject_summary(content, summary), body)
...
def main() -> int:
    ...
    stats = {"ok": 0, "fail": 0, "skip": 0, "google": 0, "unreadable": 0}
    fails: list[str] = []
    done = 0
    ...
    print(f"\nDone: {done} notes")
    print(f"  ok        {stats['ok']}   (正文+摘要已写入)")
    print(f"  skip      {stats['skip']}   (已有 summary)")
