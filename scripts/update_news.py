...
#    七细类（2026-08-23 三层重构 + 2026-08-26 v5.0 社会创新）。
#    v5.1（2026-08-26）细化：每细类补 25 分「重要级」中间档，30 档只放真里程碑词，
...
CONTENT_STRENGTH_RULES: dict[str, list[tuple[int, list[str]]]] = {
    ...
    # 基础研究：判据=发现价值＋发表层级（颠覆发现/诺奖>重要发表>常规论文）
    "基础研究": [
        (30, ["突破性进展", "新模型", "新方法", "首次", "世界首个", "里程碑",
              "发现", "颠覆", "诺奖", "诺贝尔", "获奖"]),
        (25, ["发表", "重大发现", "重要进展", "原创", "封面论文"]),
        (20, ["研究", "论文", "算法", "理论", "模型", "方法学", "实验",
              "benchmark", "数据集", "机理", "机制", "paper", "research", "study"]),
    ],
    # 社会创新：判据=机制层级＋首创性（国家级机制创新>地方试点倡导>常规倡导）
    "社会创新": [
        (30, ["机制创新", "制度创新", "模式创新", "全国首个", "全球首个",
              "国家级试点", "全民参与"]),
        (25, ["绿色消费", "低碳消费", "绿色生活", "低碳生活", "公众参与",
              "行为改变", "试点", "示范", "倡导", "倡议"]),
        (20, ["垃圾分类", "无废城市", "循环消费", "新业态", "社区", "生活方式",
              "共享", "绿色出行"]),
    ],
    # 企业经营：判据=里程碑＋规模（世界级首台套/超大规模>重要投产签约>常规经营）
    "企业经营": [
...
def score_content_strength(sub_dimension: str, title: str, summary: str) -> int:
    ...
    v5.1（2026-08-26）：改用 `_kw_hit`（词边界）匹配——英文完整词按词边界
    （cop 不再误命中 copper、system 不再误命中 ecosystem、test 不再误命中 latest），
    中文/含空格短语/故意前缀词保持子串。
    """
    text = f"{title or ''} {summary or ''}".lower()
    rules = CONTENT_STRENGTH_RULES.get(sub_dimension, CONTENT_STRENGTH_RULES["企业经营"])
    for score, kws in rules:
        if any(_kw_hit(text, kw.lower()) for kw in kws):
            return score
...
def _kw_hit(text: str, kw: str) -> bool:
    ...
    右边界用 `(?![a-z])`（仅禁字母后缀，允许数字后缀）——这样缩写+届数/型号
    能命中（cop30/cop29/gpt5/ndc3.0/ev6），但仍拒绝字母后缀误匹配
    （cop→copilot/copper/copy、gene→generation、ev→every）。
    """
    if not _ASCII_ONLY_RE.match(kw) or kw in _PREFIX_KWS:
        return kw in text
    # 词干 + 常见后缀（s/es/ing/ed/d 覆盖复数与动词变形），左边界禁字母数字，右边界只禁字母
    return re.search(
        r"(?<![a-z0-9])" + re.escape(kw) + r"(?:s|es|ing|ed|d)?(?![a-z])", text
    ) is not None
