...
- [ ] ~~趋势图还是不好看。~~
- [ ] 区域选择国际的时候，分段全部，周期是“日”，此时显示的都是专家解读啊。这个不对。
- [ ] 24小时高分浓缩版，还需要优化。并且和搜索框放在一起容易混淆（这个应该有搜索键）。
- [x] 把摘要二字，换成：展开摘要。
- [x] 新闻的标签，要放到数字后面的行业标签后面，两个标签并列显示。 
- [x] 国际和国内的区域标签还是不对。
- [ ] 要用ai给库里的东西互相打双链接啊。
- [x] **数据质量 QA 系统（2026-08-21）**：scripts/qa_check.py 服务器每次抓取后自动跑（sync.sh 1.7 步骤），检查 A标题/B摘要/C翻译/D时间/E链接/F数据 六类，100 分制评分输出 data/qa-report.json+md（只报告不修复）；**工作流：每次会话开始 agent 先读 qa-report.md 汇报问题，老温定任务**。首轮修复（72.5→88.5）：X 平台摘要 &amp; 未解码（fetch_x unescape + 全局 _clean_summary）、Google News 纯日期标题过滤、中国环境报 {{content.*}} 模板残留（含未闭合/空标签/版权页脚）、中国能源报"注册"确认为误报（注册资本，词表移除）。**下一轮待处理（源头提取层）**：chinanecc 摘要打印工具栏、cneeex 摘要"登录"导航残留、Mongabay/Google News 个别条翻译失败、存量"08/19/2026"日期条目待自然淘汰
- [ ] 虎嗅的摘要显示不正确。

推进的这么费劲，难道要用codex？明天试试吧……
而且github上一大堆别人做的。我为什么不采取用现成的再改呢……
- 我不就是想用qmd，想用obsidian的关系图谱吗。这是我的两个诉求。 
- qmd的问题并不大；
...
- [x] 主页信息表达重构（2026-08-19 已实施并部署；2026-08-20 四维改名为 政策/产业/市场信号/AI）：三段式布局——① 顶部控制（主题=四维 政策/产业/市场信号/AI + 全部 / 时间段 日周月 / 区域 国内国际 / 📋 一键复制概要，含 ywm.life 网站宣传 + 摘要清洗）；② 中部列表（重要性/新到旧 两种排序 + 每页 10/20/50 条 + 紧凑分页，卡片显示四维徽章 + 主题标签 chip，不显示地域/政策类型管理标签）；③ 底部主题关系图谱（ECharts graph 力导向，节点=主题标签、边=同一篇新闻内主题标签共现，按当前四维筛选，节点大小=出现次数，只显示主题标签不含管理标签）。数据层：update_news.py 新增 extract_topic_tags()（仅 TOPIC_RULES 主题标签，不含管理标签）+ items[].topics 字段导出 + merge_history 回填；scripts/backfill_topics.py 回填存量。已部署 ywm.life：scp 前端三件（v22/v12）+ backfill_topics.py，服务器回填 1256 条，线上 12 个主题标签图谱正常
