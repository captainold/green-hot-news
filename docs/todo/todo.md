...
- [x] **A4 TRL 词表补文档**：08-24 扩展词补进词典 2.3 + 打分文档 TRL 表（代码 7-9/4-6/1-3 三档均有新增）（2026-08-26 完成：词典 2.3 与打分体系标准.md v5.0 TRL 表均已含 机组/电站/电芯/中标/通过评价/首台/首套/首创 等扩展词）
- [x] **A5 待修**：词典 6.2 qmd frontmatter 示例重写（dimension 中文 + layer 独立字段 + taxonomy 展平 4 字段）【部分完成：dimension 注释已改 v5.0 中性命名；layer 独立字段待 export_qmd 实际格式核对】
- [x] **B1 待修**：AGENTS.md 内容过时（v2.2 五维→v4.0 六维、四维→三层六细类）（2026-08-26 完成：AGENTS.md 全面更新——v5.0 六维模型 + 三层七细类 + AI 分流 + 新判定优先级）

### 🔄 三层体系重构 v5.0（2026-08-26 下午，老温拍板 + 审核 todo 后当日执行完毕）

> 完整执行清单与记录：`docs/todo/2026-08-26 三层重构todo（政策-创新-产业）.md`；commit 2a14936（重构）+ 02edea8（社会创新源）+ 9101948（数据入库）

- [x] **三层中性命名**：绿色政策/绿色产业/科技创新 → **政策/创新/产业**（去掉「绿色」前缀，保持中性；展示顺序按创新价值链：政策（为什么）→ 创新（可能吗）→ 产业（成了吗））
- [x] **七细类**：新增**社会创新**（制度/机制/模式/消费/行为创新）→ 创新层；强度档位 30/20/默认 8
- [x] **AI 按技术阶段分流**（`_ai_stage_sub`）：论文/研究报告→创新·基础研究；模型/产品发布→产业·企业经营；其余研发→创新·技术研发（16 条示例断言全绿）
- [x] **碳普惠/碳账户/绿色金融产品 → 产业层**（老温决策：已落地应用；贷款类词入金融资本防 A1 抢先）
- [x] **非 AI 源学术载体窄词路径**（论文/学术/arxiv/机理→创新·基础研究）
- [x] **历史数据迁移**：本地 1879 条 + 服务器 2350 条重算，0 异常；三层分布均衡（本地 政策 435/创新 407/产业 520）
- [x] **社会创新养源（发现 1 处置）**：接入 Nature Sustainability（学术顶刊，ACADEMIC_SITES 直通）+ Hot or Cool Institute（生活方式研究所，SOCIAL_THINKTANK_SITES 直通）+ UNEP（政策库·国际组织）；SOCIAL_INNOVATION_KW 补英文词；74 源全 OK；服务器 latest-24h 社会创新 13 条（维度激活）
- [x] **前端/wiki/文档同步**：DIMS 三层 + 七细类标签 + CSS；wiki 三层导航；打分体系标准.md v5.0 + 本体与标签词典 9.1；AGENTS.md（B1 清）
- [x] **部署**：本地 + 服务器 74/74 源 0 failed；服务器迁移 + 17:00 timer 轮次验证（QA 92.0 A）

> ⚠️ 已知边界：社会创新英文词（lifestyle 等）可能误伤消费类产业新闻（跑分布再调）；UNEP RSS 含 2023-24 老报告（窗口过滤已处理）；AI 治理/安全信号词暂留技术研发 30 档；**社会创新源会持续累积**（hotorcool 旧文被 62 天窗口滤掉属正常）
...
### P1（一致性收尾 + 数据治理）
- [x] **A4 TRL 词表补文档**：08-24 扩展词补进词典 2.3 + 打分文档 TRL 表（2026-08-26 完成）
- [ ] **A5 词典 frontmatter 示例重写**：6.2 节改 dimension 中文 + layer 独立字段 + taxonomy 展平 4 字段（dimension 注释已改，layer 独立字段待核对）
- [x] **B1 AGENTS.md 内容过时修正**：v2.2 五维→v5.0 六维、四维观察窗口→三层七细类描述（2026-08-26 完成）
